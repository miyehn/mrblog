...
**注意：这个win branch里的是给win的源码，这个文件是win版的说明，mac版的码和说明在master branch里**

学一点基本的linux命令行操作，会从命令行去各个文件夹，会从命令行里执行命令，知道管理文件的基本术语，绝对路径，相对路径等。

会用一点github，有一个免费的github账号。如果之前没有在本地用过github的话就从命令行按顺序：
```
...
按顺序安装这几个东西：
- Ubuntu （[官方安装说明](https://docs.microsoft.com/en-us/windows/wsl/install-win10)，芒果的实际安装步骤如下）
	- 开始菜单搜索 "turn windows features on or off"
	- 勾选 "windows subsystem for linux", 确定然后重启
	- microsoft store里找到Ubuntu，获取然后安装
	- 安装完后首次打开要设置登录名和密码，随便设了...
- [linuxbrew](https://linuxbrew.sh/)，把install下面的那些安装命令一个个复制到linux命令行然后回车（一共5个）
- [node](https://nodejs.org/) (`brew install node`)
- [imagemagick](https://imagemagick.org/index.php) (`brew install imagemagick`)
- pip3 ([步骤在这儿](https://linuxize.com/post/how-to-install-pip-on-ubuntu-18.04/))
- [pyperclip](https://pypi.org/project/pyperclip/)（`pip3 install pyperclip`）
- Markdown编辑器[typora](https://typora.io/)，安装到默认路径不要改。
...
```
$ git clone --single-branch --branch win https://github.com/miyehn/mrblog.git
```
...
**ps：后两个路径，都是针对linux来说的绝对路径，开头都应该是/mnt/。不确定的话就从命令行去到附近然后通过`pwd`找**
...
$ mrblog new <不带后缀的文件名>
```
会新建博文文件，并且文件被命名为这个自定义的文件名。在文件夹里辨识度高些吧。
...
$ mrblog find
```
会开始互动对话，帮助从文件夹找到某篇博文然后对它操作，可以选择重新打开它编辑，或者重命名，或者删除它。
...
流程：命令行去到博客文件夹(`cd $MRBLOG_SCRIPTS/..`)然后
```
$ npm start
```
会看到浏览器打开博客预览。
...
替换Friends下面的`friends list↓`为 "友链"，"朋友们" 或者别的什么表示友链的文字，替换`none`为友链博客名，替换博客名前的地址为友链地址。
...
### 修改博客外观
打开`<博客文件夹>/src/theme/style/style.css`，修改字体，字号，行间距，博客颜色等变量。

或者修改/重写整个`<博客文件夹>/src/theme`文件夹里的东西，相当于制作新的主题。
